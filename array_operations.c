...
{ unsigned int i; for (i = 0; i < n; i++) { v[i] = z; } }

void carrCopy(unsigned int n, Carray from, Carray to)
{ unsigned int i; for (i = 0; i < n; i++) { to[i] = from[i]; } }

void carrRPart(unsigned int n, Carray v, Rarray vreal)
{ unsigned int i; for (i = 0; i < n; i++) { vreal[i] = creal(v[i]); } }

void carrIPart(unsigned int n, Carray v, Rarray vimag)
{ unsigned int i; for (i = 0; i < n; i++) { vimag[i] = cimag(v[i]); } }
...
{ unsigned int i; for (i = 0; i < n; i++) { v_conj[i] = conj(v[i]); } }

void carrAbs(unsigned int n, Carray v, Rarray vabs)
{ unsigned int i; for (i = 0; i < n; i++) { vabs[i] = cabs(v[i]); } }
...
{ unsigned int i; for (i = 0; i < n; i++) { ans[i] = cexp(z * v[i]); } }
